...
function readData() {
  try {
    ...
    return {
      ...
      overlay: { showWheel: true, onlyOnGift: true }
    };
  }
}
...
app.post('/api/punishments', (req,res)=>{
  ...
  if (!isNaN(fs) && fs >= 0.5 && fs <= 2) data.wheel = Object.assign({}, data.wheel || {}, { fontScale: fs });
  }
  if (overlay && typeof overlay === 'object') {
    const patch = {};
    if (typeof overlay.showWheel === 'boolean') patch.showWheel = overlay.showWheel;
    if (typeof overlay.onlyOnGift === 'boolean') patch.onlyOnGift = overlay.onlyOnGift;
    if (Object.keys(patch).length) data.overlay = Object.assign({}, data.overlay || {}, patch);
  }
  writeData(data);
  res.json({ ok: true });
});
