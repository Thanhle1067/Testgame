...
// Hype Train / Combo System
let comboCount = 0;
let lastActionTime = 0;
const COMBO_TIMEOUT = 5000; // Reset combo if no action for 5 seconds
...
setInterval(() => {
  ...
  // Custom logic: check combo timeout
  if (comboCount > 0 && (Date.now() - lastActionTime > COMBO_TIMEOUT)) {
...
app.all('/api/add', (req, res) => {
  ...
  res.json({ ok: true, method: req.method, seconds, lastDelta, lastType, lastEventId, active, comboCount });
});
...
app.get('/api/state', (req, res) => {
  // Check combo timeout on state read as well to be safe, or just return current
  if (comboCount > 0 && (Date.now() - lastActionTime > COMBO_TIMEOUT)) {
    comboCount = 0;
  }
  res.json({ ok: true, seconds, lastDelta, lastType, lastEventId, active, comboCount });
});
