// server.js — overlay OBS, nhận cả GET và POST (form/json) cho /api/add, /api/rescue, /api/gift
...
app.use(express.urlencoded({ extended: true })); // nhận POST form
app.use(cors());
...
function pickSec(obj, def = null) {
  return toInt(obj?.sec ?? obj?.Sec ?? obj?.SEC ?? obj?.value1 ?? obj?.amount, def);
}
function pickAmount(obj, def = 1) {
  return toInt(obj?.amount ?? obj?.coins ?? obj?.likeCount ?? obj?.repeatCount, def);
}
function pickUsername(obj, def = 'Anonymous') {
  return obj?.u || obj?.username || obj?.tikfinityUsername || obj?.nickname || obj?.nickName || def;
}
...
// ==== /api/gift (giữ tương thích cũ) ====
app.get('/api/gift', (req, res) => {
  const username = pickUsername(req.query);
  const amount = pickAmount(req.query, 1);
...
app.post('/api/gift', (req, res) => {
  const username = pickUsername(req.body);
  const amount = pickAmount(req.body, 1);
...
app.post('/api/add', (req, res) => {
  const username = pickUsername(req.body);
  const secBody = pickSec(req.body, null);
  const amount = pickAmount(req.body, 1);
  const sec = secBody !== null ? secBody : null;
  res.json(emitGift({ username, amount, sec, type: 'normal' }, req));
});
...
app.get('/debug', (req, res) => {
  ...
  // chỉ 1 backtick ngoài, bên trong dùng chuỗi bình thường => tránh lỗi deploy
...
.row{display:grid;grid-template-columns:160px 1fr;gap:8px;padding:8px;border-bottom:1px solid #24314d}
...
<div class="top">
  <h1 style="margin:0;font-size:20px">Debug Webhooks</h1>
  <span class="pill">/api/add (GET/POST)</span>
  <span class="pill">/api/rescue (GET/POST)</span>
...
<script>
  ...
  function row(d){
    ...
    el.innerHTML =
      '<div>' + d.ts + '</div>' +
      '<div class="mono"><b>' + d.kind + '</b> ' + d.method + ' ' + d.path +
      '<br/>query: ' + JSON.stringify(d.query||{}) +
      '<br/>body: ' + JSON.stringify(d.body||{}) +
      '<br/>payload: ' + JSON.stringify(d.payload||{}) +
      '</div>';
    list.prepend(el);
  }
