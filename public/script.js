...
(async function(){
  ...
  function connectWS() {
    ...
    ws.onmessage = (ev)=> { try { const msg = JSON.parse(ev.data); if (msg.type === 'gift') enqueueGift(msg.payload || {}); } catch (e) {} };
  ...
  try {
    ...
    if (Array.isArray(qjson.queue)) qjson.queue.forEach(item => enqueueGift(item));
  ...
  function processNext(){
    ...
    runSpinOnce(next, ()=>{
      processing = false;
      setTimeout(processNext, 200); // small gap
    });
  }
  ...
  async function pollQueueOnce(){
    try{ const res = await fetch('/api/queue', { cache: 'no-store' }); const { queue } = await res.json();
      if (!processing && localQueue.length===0 && Array.isArray(queue) && queue.length>0){ queue.forEach(item => enqueueGift(item)); }
    }catch(e){}
  }
  function startQueuePolling(){ if (_pollTimer) return; _pollTimer = setInterval(pollQueueOnce, 2000); }
  startQueuePolling();
})();
