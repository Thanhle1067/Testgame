...
(async function(){
  ...
  function speak(text, cb_force_hide) {
    try {
      ...
      u.onend = () => { if (cb_force_hide) cb_force_hide(); };
  ...
  function hideOverlay() {
    if (!wrapper) return;
    if (hideDelaySec <= 0) wrapper.classList.add('hidden');
    else setTimeout(()=> wrapper.classList.add('hidden'), hideDelaySec * 1000);
  }
