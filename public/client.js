...
function showPrison(){
  if (!prisonShown) {
    prisonShown = true;
    prisonEl.classList.add('show');
    prisonEl.classList.remove('hidden');
  }
}
function hidePrison(){
  prisonEl.classList.remove('show');
  prisonShown = false;
  setTimeout(() => prisonEl.classList.add('hidden'), 800);
}

function showBonusText(text){
  bonusTextEl.textContent = text;
  bonusTextEl.classList.add('show');
  setTimeout(() => bonusTextEl.classList.remove('show'), 900);
}

// Tick: only when > 0; auto-hide at 0
setInterval(() => {
...
socket.on('gift', (data) => {
  ...
  if (isRescue) delta = -delta;

  remainingSeconds = Math.max(0, remainingSeconds + delta);

  if (delta > 0) {
    showBonusText('+' + delta + 's');
    try { ting.currentTime = 0; ting.play(); } catch(e) {}
  } else if (delta < 0) {
    showBonusText(delta + 's');
  }
