const socket = io();
let remainingSeconds = 0; // start 0
...
function renderSmall(){ const m = Math.floor(Math.max(0, remainingSeconds) / 60); const s = Math.max(0, remainingSeconds) % 60; timerEl.textContent = `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`; }
...
setInterval(() => { if (remainingSeconds > 0) { remainingSeconds -= 1; if (remainingSeconds <= 0) { remainingSeconds = 0; hidePrison(); } renderBig(); renderSmall(); } }, 1000);
socket.on('gift', (data) => { const amount = parseInt(data?.amount || 1, 10); const secOverride = data?.sec ? parseInt(data.sec, 10) : null; const isRescue = data?.type === 'rescue'; let delta = Number.isFinite(secOverride) ? secOverride : (10 * (isNaN(amount) ? 1 : amount)); if (isRescue) delta = -delta; remainingSeconds = Math.max(0, remainingSeconds + delta); if (delta > 0) { showBonusText(`+${delta}s`); try { ting.currentTime = 0; ting.play(); } catch(e) {} } else if (delta < 0) { showBonusText(`${delta}s`); } if (remainingSeconds > 0) showPrison(); else hidePrison(); renderBig(); renderSmall(); });
