...
function showPrison(){ if (!prisonShown) { prisonShown = true; prisonEl.classList.add('show'); } }
function hidePrison(){ prisonEl.classList.remove('show'); prisonShown = false; }
...
socket.on('gift', (data) => { const amount = parseInt(data?.amount || 1, 10); const secOverride = (data?.sec !== undefined && data.sec !== null) ? parseInt(data.sec, 10) : null; const isRescue = data?.type === 'rescue'; let delta = Number.isFinite(secOverride) ? secOverride : (10 * (isNaN(amount) ? 1 : amount)); if (isRescue && delta > 0) delta = -delta; remainingSeconds = Math.max(0, remainingSeconds + delta); if (delta > 0) { showBonusText('+' + delta + 's'); try { ting.currentTime = 0; ting.play(); } catch(e) {} } else if (delta < 0) { showBonusText(delta + 's'); } if (remainingSeconds > 0) showPrison(); else hidePrison(); renderBig(); });
