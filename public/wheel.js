...
class Wheel {
  ...
  draw() {
    ...
    for (let i=0;i<this.labels.length;i++){
      ...
      ctx.closePath();
      // alternate fill without fixed colors
  ...
  spinToIndex(index, cb) {
    // rotate so that the chosen index aligns with pointer at top
    const targetAngle = (Math.PI * 1.5) - (index * this.sectorAngle) - (this.sectorAngle/2);
    // add random full rotations for flair
    const extra = Math.PI * 2 * (3 + Math.floor(Math.random()*3)); // 3-5 full turns
  ...
  indexAtPointer() {
    // pointer at top (Math.PI*1.5)
    let a = (Math.PI*1.5 - this.rotation) % (Math.PI*2);
    if (a < 0) a += Math.PI*2;
    const idx = Math.floor(a / this.sectorAngle);
    return idx;
  }
}
window.Wheel = Wheel;
