...
class Wheel {
  ...
  setFontScale(scale){ this.fontScale = Math.max(0.5, Math.min(2, Number(scale)||1)); }
  setLabels(labels) {
    this.labels = labels;
    this.sectorAngle = (Math.PI * 2) / labels.length;
    this.rotation = 0;
  ...
  draw() {
    ...
    for (let i=0;i<this.labels.length;i++){
      ...
      ctx.textAlign = 'right';
      ctx.fillStyle = '#111';
      const base = 24 * this.fontScale;
      let size = Math.max(12, Math.min(48, Math.round(base)));
      const maxWidth = r * 0.6;
      ctx.font = `bold ${size}px system-ui, sans-serif`;
      while (ctx.measureText(this.labels[i]).width > maxWidth && size > 12) {
        size -= 1;
        ctx.font = `bold ${size}px system-ui, sans-serif`;
      }
